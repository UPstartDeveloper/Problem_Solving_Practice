...
class Solution:
    ...
    def scheme_valid(
        self, crimes: tuple[int], total_members: int, minProfit: int
    ) -> bool:
        # init the is_valid
        is_valid = False
    ...
    def profitableSchemes(
        self, total_members: int, minProfit: int, group: List[int], profit: List[int]
    ) -> int:
        """
        n people
        ...
        def generate_schemes(current_scheme: tuple[int], crimes_available: List[int]):
            ...
            is_valid, total_profit, group_size = self.scheme_valid(
                current_scheme, total_members, minProfit
            )
            if is_valid is True:
