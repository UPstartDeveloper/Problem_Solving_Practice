class Solution:
    ...
    """
    CP = CLOCK_POSITIONS = [
        degree for degree in range(0, 360, 30)
    ]
    ...
    def compute_angle_between_hour_and_min(self, time: str) -> float:
        ...
        def _compute_angle(start, end):
            ...
            end_degrees = (
                start_degrees + (30 * ((end - end_int) + hours_in_between))
            )
            # C: return diff
        ...
        if not isinstance(time, str) or ":" not in list(time):
            raise ValueError(f"Expected a time string in hh:mm format, actual is {time}.")
        ...
        if not -1 < minute < 60 or not isinstance(minute, int):
            raise ValueError(f"Arg for minute = {minute} is not an int or outside of allowed range of 0-59.")
